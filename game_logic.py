import checkers_exception as exc
import random
...
class Player:
        ...
        for step in list_step:
            for x in range(delta * step,   delta * step + 1, 2):
                for y in range(-1 * step, step + 1, 2):
                    if self.is_correctness_coord(x + chip.pos_x, y + chip.pos_y) and \
                            (self.field[x + chip.pos_x][y + chip.pos_y] == 0):
                        self.active_chip_list_move.append([x + chip.pos_x, y + chip.pos_y])
    ...
    def make_jump(self, pos_x, pos_y, party):
        ...
        if (abs(self.active_chip.pos_x - pos_x) + abs(self.active_chip.pos_y - pos_y)) % 2 == 0\
                and is_correct_jump and self.field[self.active_chip.pos_x][self.active_chip.pos_y].is_king:
...
class PlayingField:
    ...
    def __init__(self, dimension, field_save=None, unusual_mod=False):
        self.init_field(dimension)
        if field_save is None:
            self.init_white(unusual_mod)
            self.init_black(unusual_mod)
        else:
    ...
    def init_white(self, unusual_mod):
        self.first_player = Player('white', self.field)
        self.put_chips(0, self.dimension // 2 - 1, 1, self.dimension, 'black', self.first_player, unusual_mod)
        self.put_chips(1, self.dimension // 2 - 1, 0, self.dimension, 'black', self.first_player, unusual_mod)

    def init_black(self, unusual_mod):
        self.second_player = Player('black', self.field)
        self.put_chips(self.dimension // 2 + (self.dimension // 2 - 1) % 2 + 1, self.dimension, 1,
                       self.dimension, 'white', self.second_player, unusual_mod)
        self.put_chips(self.dimension // 2 + 1 + (self.dimension // 2) % 2, self.dimension, 0,
                       self.dimension, 'white', self.second_player, unusual_mod)

    def put_chips(self, start_x, finish_x, start_y, finish_y, enemy_party, player, unusual_mod):
        for x in range(start_x, finish_x, 2):
            for y in range(start_y, finish_y, 2):
                is_unusual = False
                if unusual_mod and random.randint(1, 10000) % 5 == 0:
                    is_unusual = True
